...
fn TerminalText(future: Resource<Arc<RwLock<Child>>>) -> Element {
    if let Some(ref sh1) = *future.read_unchecked() {
        ...
        tokio::spawn(async move { //pushing commands to stdin
        ...
        let _ = use_resource(move || {
            ...
            async move {
                loop {
                    ...
                    let Ok(buf) = timeout(Duration::from_millis(200), outBuf.fill_buf()).await
                    else {
                    ...
                    onkeydown: move |event|{
                        ...
                        async move {
                            if event.key() == Key::Enter {
                                dbg!("Enter pressed");
                                if tx.send(format!("{}\n", input_text.peek().to_string())).await.is_err() {
