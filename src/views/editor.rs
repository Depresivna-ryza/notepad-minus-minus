...
pub fn Editor(tabs: Signal<Tabs>) -> Element {
    let text: Signal<Option<Text>> = use_signal(|| match tabs.read().current_file {
    ...
    let caret_line: Memo<usize> = use_memo(move || match text.read().clone() {
...
pub fn EditorText(tabs: Signal<Tabs>, text: Signal<Option<Text>>) -> Element {
    ...
    let text: Option<Text> = match *current_file.read() {
        Some(ref file_path) => read_to_string(file_path).ok().map(Text::new),
        None => None,
    };
